...
public final class JdbcAnalysisRepository implements AnalysisRepository {
    ...
    public List<VersionAnswer> loadVersionData(UUID datasetVersionId) {
        try (var connection = connections.open();
             var statement = connection.prepareStatement("""
                     SELECT s.response_id, s.question_id, s.value_json, s.excluded, q.question_type
                     FROM version_answer_snapshots s JOIN questions q ON q.id = s.question_id
        ...
        transactions.inTransaction(connection -> {
            try (var statement = connection.prepareStatement("""
                    INSERT INTO analyses(id, study_id, dataset_version_id, method, plan_json, result_json,
                        sample_size, warnings_json, source, model_metadata_json, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """)) {
    ...
    private static String modelMetadataJson(ModelMetadata metadata) {
        return "{\"model\":\"" + SnapshotJson.escape(metadata.model()) + "\",\"runtime\":\""
                + SnapshotJson.escape(metadata.runtime()) + "\",\"promptVersion\":\""
                + SnapshotJson.escape(metadata.promptVersion()) + "\"}";
    }
    ...
    private static String planJson(AnalysisPlan plan) {
        ...
        for (int index = 0; index < plan.filters().size(); index++) {
            ...
            builder.append("{\"questionId\":\"").append(filter.questionId()).append("\",\"operator\":\"")
                    .append(filter.operator().name()).append("\",\"value\":\"").append(SnapshotJson.escape(
    ...
    private static String frequencyJson(AnalysisResult.Frequency value) {
        ...
        for (int index = 0; index < value.categories().size(); index++) {
            ...
            if (index > 0) builder.append(',');
            builder.append("{\"value\":\"").append(SnapshotJson.escape(category.value())).append("\",\"count\":")
    ...
    private static String groupComparisonJson(AnalysisResult.GroupComparison value) {
        return "{\"type\":\"group_comparison\",\"groupALabel\":\"" + SnapshotJson.escape(value.groupALabel())
                + "\",\"groupACount\":" + value.groupACount() + ",\"groupAMean\":" + value.groupAMean()
                + ",\"groupASD\":" + value.groupASD() + ",\"groupBLabel\":\"" + SnapshotJson.escape(value.groupBLabel())
    ...
    private static String jsonArray(List<String> values) {
        ...
        for (int index = 0; index < values.size(); index++) {
            if (index > 0) builder.append(',');
            builder.append('"').append(SnapshotJson.escape(values.get(index))).append('"');
        }
        return builder.append(']').toString();
    }
}
