...
public final class FindingsWorkspaceView {
    ...
    private void review(Finding finding) {
        ...
        if (finding.status() == FindingStatus.APPROVED) {
            content.getChildren().add(new Label("Changing wording returns this finding to Draft and requires reapproval."));
        }
        ...
        if (finding.status() != FindingStatus.APPROVED) {
            ...
            approve.disableProperty().bind(editor.textProperty().isNotEqualTo(finding.text()));
            content.getChildren().add(new Label("Save changed wording before approving."));
