...
public final class ImportWorkspaceView {
    ...
    public ImportWorkspaceView(Study study, DatasetImportService service, Async async, Runnable onImported,
        ...
        root.getChildren().addAll(eyebrow, heading, hint, form, status, columnsTitle, columnsScroll, importButton);
    ...
    private void runImport() {
        ...
        importButton.setDisable(true);
        async.run(() -> service.importInto(study.id(), title.getText(), preview, columns), this::showResult, failure -> {
            importButton.setDisable(false);
    ...
    private void showResult(ImportResult result) {
        importButton.setDisable(false);
        var message = new StringBuilder(result.importedCount() + " row(s) imported.");
        ...
        var alert = new Alert(result.skippedCount() > 0 ? Alert.AlertType.WARNING : Alert.AlertType.INFORMATION);
