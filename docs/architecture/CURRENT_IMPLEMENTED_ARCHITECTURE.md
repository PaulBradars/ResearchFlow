...
interpreted only by `QualityReviewService`. `Correct` reuses `DatasetCorrectionService`'s already
audited transaction and then marks the issue resolved; `Exclude`, `Accept`, and `Defer` each update
...
`VersionRepository.createSnapshot` snapshots every current answer and response status for the Study
into `version_answer_snapshots` under a new `dataset_versions` row (parent = previous active version,
`is_active=1`), deactivating the prior active version in the same transaction. `restore` rewrites live
`answers`/`responses` to match an older version's snapshot, then calls the same snapshot routine to
record the result as a brand-new active version — earlier versions, including the one restored from,
...
could be derived) with `analysisId`/`datasetVersionId` fixed permanently; `edit` only ever changes
`text`. `ReportService.compose` is a **fresh composition on every call** — Study metadata, form/
...
`MigrationRunner` registers V001 through V004, recording each once in `schema_migrations`. Neither
Phase 5 nor Phase 6 required a new migration.
...
The 91-test suite covers migrations and foreign keys; generic rollback; Study/Form persistence and
